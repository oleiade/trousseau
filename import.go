...
func ImportStore(src, dest *EncryptedStore, strategy ImportStrategy) error {
    switch strategy {
    case IMPORT_YOURS:
        for key, value := range src.DataStore.Container {
            if _, ok := dest.DataStore.Container[key]; !ok {
                dest.DataStore.Container[key] = value
            }
        }
    case IMPORT_THEIRS:
        for key, value := range src.DataStore.Container {
    ...
    case IMPORT_OVERWRITE:
        dest.DataStore.Container = src.DataStore.Container
    }
