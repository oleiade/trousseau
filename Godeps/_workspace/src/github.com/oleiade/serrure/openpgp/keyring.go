...
func ReadPubRing(path string, keyIds []string) (*openpgp.EntityList, error) {
	...
	if len(hprefs) == 0 {
		return nil, NewOpenPGPError(
			...
			"No common hashes for encryption keys",
		)
	}
	if len(sprefs) == 0 {
		return nil, NewOpenPGPError(
			ERR_DECRYPTION_CIPHERS,
			"No common symmetric ciphers for encryption keys",
		)
	}
...
func ReadKeyRing(path string) (*openpgp.EntityList, error) {
	...
	if err != nil {
		return nil, NewOpenPGPError(
			ERR_KEYRING,
			fmt.Sprintf("Unable to open gnupg keyring: %v", err),
		)
	}
	defer f.Close()
	...
	if err != nil {
		return nil, NewOpenPGPError(
			ERR_KEYRING,
			fmt.Sprintf("Unable to read from gnupg keyring: %v", err),
		)
	}
