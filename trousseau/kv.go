...
func (kvs *KVStore) Keys() []string {
	index := 0
	keys := make([]string, len(kvs.Container))

	for key, _ := range kvs.Container {
		keys[index] = key
		index++
	}
...
func (kvs *KVStore) Items() map[string]interface{} {
	...
	for key, value := range kvs.Container {
		items[key] = value
	}
