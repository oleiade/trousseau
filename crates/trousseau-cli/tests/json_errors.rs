...
fn json_mode_unimplemented_subcommand_prints_json_error_only() {
    ...
    // `migrate` is still unimplemented (step 3.8): `edit` served this
    // role until step 3.7 implemented it, `run` until step 3.6, `export`
    // until step 3.5, `rekey` until step 3.4, and `set` before that
    // (step 3.3).
    let assert = env
        .command()
        .args(["--json", "migrate", "source"])
        .assert()
        .failure();
